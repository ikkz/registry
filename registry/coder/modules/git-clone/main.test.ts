...
describe("git-clone", async () => {
  ...
  it("fails when pre-clone script returns non-zero exit code", async () => {
    const state = await runTerraformApply(import.meta.dir, {
      agent_id: "foo",
      url: "fake-url",
      pre_clone_script: "exit 1",
    });
    const output = await executeScriptInContainer(state, "alpine/git");
    expect(output.exitCode).toBe(1);
    expect(output.stdout).toContain("Running pre-clone script...");
    expect(output.stdout).not.toContain("Cloning");
  });

  it("fails when post-clone script returns non-zero exit code", async () => {
    const state = await runTerraformApply(import.meta.dir, {
      agent_id: "foo",
      url: "fake-url",
      post_clone_script: "exit 1",
    });
    const output = await executeScriptInContainer(state, "alpine/git");
    expect(output.exitCode).toBe(1);
    expect(output.stdout).toContain("Running post-clone script...");
